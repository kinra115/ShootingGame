...
public class ShootingView extends SurfaceView
    ...
    public ShootingView(Activity activity) {
        ...
        segmentFont = Typeface.createFromAsset(getContext().getAssets(),"segments.ttf");

        nikoFont = Typeface.createFromAsset(getContext().getAssets(),"niko.ttf");
    }
    ...
    public void drawScore() {
        ...
        g.setTextSize(30);
        g.setScoreTypeface(segmentFont);
        g.drawText(num2str(score, 6),200+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);

    }
    
    //スレッドの処理
    public void run() {
        final int sampleMax = 60;
        long[] fpsArray = new long [sampleMax];
        int sampleSteps = 0;
        ...
        while(thread != null) {
            ...
            g.setScoreTypeface(nikoFont);
            g.drawText("SCORE ", 30+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);
            g.drawText(num2str((int)offsetTime,2), 400+g.getOriginX(), 10-(int)g.getFontMetrics().ascent);
            g.unlock();
